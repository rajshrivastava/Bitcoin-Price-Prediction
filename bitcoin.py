...
def similarity(a, b):
    ...
    denominator = M*a.std(skipna=True)*b.std(skipna= True)
...
def computeDelta(wt, X, Xi):
    ...
    for i in range(len(Xi)):
        exponent_term = math.exp(wt*similarity(x, Xi.iloc[i,:len(X)-1]))
        numerator += Xi.iloc[i,-1]*exponent_term
        denominator += exponent_term
